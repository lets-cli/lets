...
@test "config_version: if config version greater than lets version - fail - require upgrade" {
  ...
  assert_failure
  assert_line --index 0 "lets: config error: config version '0.0.3' is not compatible with 'lets' version '0.0.2'. Please upgrade 'lets' to '0.0.3' using 'lets self upgrade' command or following documentation at https://lets-cli.org/docs/installation'"
}
